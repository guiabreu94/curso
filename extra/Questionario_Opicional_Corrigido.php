          ...
          <TR>
            <TD ALIGN=left VALIGN=middle BGCOLOR=#ffffff><BLOCKQUOTE>
              <FORM ACTION="correcao.php" METHOD="post">
               ...
               check=0;
                 var questao = new Array(<?php echo $num_perguntas  ?>);
               var i;
               for(i=0;i<<?php echo $num_perguntas  ?>;i++){
                   questao[i]=0;
               }
               </script>
              ...
              *   que o aluno ja respondeu a uma pergunta e não deixar o reset limpar*/
                      var j;
                      for(j=0;j<<?php echo $quantidade_perguntas ?>+1;j++){
                   redefinir[j]=0;
               }
                      </script>
                      <?php
...
<?php echo '<input type="button" value="reset" onclick=limpar'.$cont.'(),atualizar()> ' ?>                  
                  ...
                  <DT>
                      ...
                      <script type="text/javascript">
                                function checar(){
                                    var k=0;
                                    var l;
                                    var questoes_respondidas=0;
                                    if(resposta!==<?php echo $questionario["Numero_questoes_necessarias"]; ?>){
                                        alert("Voce não respodeu a quantidade de questões necessarias.");
                                        return false;
                                    }else{
                                   
                                        for(k=0;k<<?php echo $num_perguntas  ?>;k+=1){
                                        ...
                                        return false; // para submit habilite esta linha
                                         k=<?php echo $num_perguntas  ?>;
                                     }} } 
                                     for(l=0;l<<?php echo $num_perguntas  ?>;l++){
                                     questoes_respondidas+= questao[l]+ ".";
                                     }
                            var passagem = document.getElementById("passagem");
                                 passagem.innerHTML ='<input type="hidden" name="questoes" value="'+questoes_respondidas+'">';  
