...
$nome=$_POST["questionario"];//nome identificador do questionário
...
$(document).ready(function(){
               ...
               ?>
                <OL>
                    ...
                    <DIV class="conteiner">
                      ...
                      $perguntas = "select p.conteudo,p.ID_pergunta,p.status from perguntas 
 ...
 echo '<input type="button" id="respostas'.$cont.'" value="responder" onclick=aparecer'.$cont.'(),post'.$cont.'(),atualizar()> ' ; 
                        
 //echo '<input type="button" value="limpar" onclick=limpar'.$cont.'(),atualizar()> '
                        ?>     

                       <SCRIPT>
                        function aparecer<?php echo $cont; ?>(){//função que faz o botao limpar aparecer somente quando o botão responder for apertado
                            var num = <?php echo $cont; ?>;
                            if(redefinir[num]===0){
                                if(resposta<<?php echo $questionario["Numero_questoes_necessarias"]; ?> ){//n deixa q o botao seja printado mesmo q todas as respostas necessárias tenham cido respondidas
          var div = document.getElementById("limpar<?php echo $cont;  ?>");//nome do id do local onde será posto o botão limpar
            
                div.innerHTML = '<input type="button" value="limpar" onclick=limpar'+<?php echo $cont;  ?>+'(),atualizar()>';}}//printa o botao limpar em limpar especifico
