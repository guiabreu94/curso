 ...
 */
include_once 'conexa_acento_200.php';//conexão com o banco
//Controle de session

$nome=$_GET["questionario"];//nome identificador do questionário
$userid=$_GET["userid"];
$UserId=$userid;
$userid='"'.$userid.'"';
$Nome=$nome;
...
//echo $userid;
//echo $userid."hello";
$sql_id_questionario ="select ID_questionario from questionario where nome like ".$nome;
$sql_id_questionario=mysqli_query($con,$sql_id_questionario);
$id_questionario=mysqli_fetch_array($sql_id_questionario);

$sql_codigo_resp = "select codigo_resp from questionario_resp where id_questionario =".$id_questionario["ID_questionario"]." and id_aluno = ".$userid."";
$sql_codigo_resp=mysqli_query($con,$sql_codigo_resp);
$codigo_resp=mysqli_fetch_array($sql_codigo_resp);

$sql_idperguntas_respondidas="select ID_pergunta from tentativas_resposta where codigo_resp = ".$codigo_resp["codigo_resp"];
$sql_idperguntas_respondidas=mysqli_query($con,$sql_idperguntas_respondidas);
$perguntas_respondidas= array(30);
$numero_perguntas=0;
while($row=mysqli_fetch_array($sql_idperguntas_respondidas)){
$perguntas_respondidas[$numero_perguntas]=$row["ID_pergunta"];
echo $row["ID_pergunta"];
$numero_perguntas++;
}

//var_dump($perguntas_respondidas);
//echo "<br><br><br>";
//echo $numero_perguntas;
//echo "<br><br><br>";
$perguntas = "select p.conteudo,p.ID_pergunta,p.status,p.foto from perguntas
              ...
              where questionario.nome like ".$nome." and ";
            //  echo $perguntas;
            //  echo "<br><br><br>";

$cont_perg=0;
for($cont_perg=0;$cont_perg<$numero_perguntas;$cont_perg++){
  //echo "alo<br>";
  if($cont_perg==$numero_perguntas-1){
$perguntas= $perguntas."p.ID_pergunta =".$perguntas_respondidas[$cont_perg]."";
}else{
  $perguntas= $perguntas."p.ID_pergunta =".$perguntas_respondidas[$cont_perg]." or ";
}
      ...
      <TD ALIGN=center VALIGN=middle BGCOLOR=#ffd700 WIDTH=70% ><TABLE CELLPADDING=5 CELLSPACING=0 BORDER=4 BGCOLOR=#ffd700  WIDTH=100%>
          ...
          <TR>
            <TD ALIGN=left VALIGN=middle BGCOLOR=#ffffff><BLOCKQUOTE>
              <FORM ACTION="correcao_outra_tentativas.php" METHOD="post">
                ...
                <OL>
                    ...
                    /*  $perguntas = "select p.conteudo,p.ID_pergunta,p.status,p.foto from perguntas
                     ...
                     // $result;
                      ...
                      $cont=0;
                       ?>
                   <?php $num_perguntas=$numero_perguntas;
                   echo $numero_perguntas;
              ...
              *   que o aluno ja respondeu a uma pergunta e não deixar o reset limpar*/
                      ...
                      for(j=0;j<70;j++){//for de inicialização do vetor
                     ...
                     ?> <?php
                      while($pergunta= mysqli_fetch_array($perguntas_cont)){//Enquanto houverem perguntas a serem trazidas do banco tal comando continuará rodando
                       ...
                       echo  "<LI>";  echo '<a class="btn btn-default">'.$pergunta["conteudo"].'</a>'; //Printa só o conteúdo da pergunta na tela do usuário
                        //numero da questao
    echo "&ensp;";
 echo '<input type="button" id="respostas'.$cont.'" class="btn btn-info" value="responder" onclick=aparecer('.$cont.','.$numero_perguntas.'),post'.$cont.'(),atualizar()> ' ; echo "<br><br>";
          ...
          var div = document.getElementById("limpar"+num);//nome do id do local onde será posto o botão limpar
                ...
                div.innerHTML = '<input type="button" value="limpar" class="btn btn-danger" onclick=limpar'+num+'(),atualizar()>';}}//printa o botao limpar em limpar especifico
                      ...
                      //  console.log(resposta);
                        ...
                        if(resposta<<?php echo $numero_perguntas; ?> ){
                           ...
                           * -Além disso gera um input do tipo hidden que irá fornece quais foram as questões que o aluno decidiu responder  */
                            ...
                            var passagem = document.getElementById("passagem");//vai na div com o id passagem e pega seu conteúdo onde será impresso o camp hidden que passaram para o php e por conseguinte para o banco quais questoes que o aluno respondeu
                                 passagem.innerHTML ='<input type="hidden" name="questoes" value="'+questoes_respondidas+'">';//campo hidden que faz a passagem das questoes respondidas para o php
                                        ...
                                        <input type="hidden" name="questionario" id="questionario" value="<?php echo $nome; ?>">
